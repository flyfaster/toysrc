...
BOOL INtDll::Init()
{
	...
	if (dw != ERROR_SUCCESS)
	{
		wxLogError(_T("GetProcAddress(%s) failed with error: %08x"), _T("NtQuerySystemInformation"), dw);
	}
	...
	if (dw != ERROR_SUCCESS)
	{
		wxLogError(_T("GetProcAddress(%s) failed with error: %08x"), _T("NtQueryObject"), dw);
	}
	...
	if (dw != ERROR_SUCCESS)
	{
		wxLogError(_T("GetProcAddress(%s) failed with error: %08x"), _T("NtQueryInformationThread"), dw);
	}
	...
	if (dw != ERROR_SUCCESS)
	{
		wxLogError(_T("GetProcAddress(%s) failed with error: %08x"), _T("NtQueryInformationFile"), dw);
	}
	...
	if (dw != ERROR_SUCCESS)
	{
		wxLogError(_T("GetProcAddress(%s) failed with error: %08x"), _T("NtQueryInformationProcess"), dw);
	}
...
{
	m_pBuffer = (UCHAR*)VirtualAlloc (0,
		//(void*)0x100000,
						BufferSize, 
						MEM_COMMIT,
						PAGE_READONLY
	...
	if (m_pBuffer==NULL)
	{
		...
		wxLogError(_T("SystemProcessInformation::SystemProcessInformation() error code: %d, description: %s"), dw, (LPCTSTR)lpMsgBuf);
		LocalFree(lpMsgBuf);
...
BOOL SystemProcessInformation::Refresh()
{
	...
	if ( nt_status=INtDll::NtQuerySystemInformation( 5, m_pBuffer, BufferSize, NULL ) != 0 )
	{
		...
		wxLogError(_T("NtQuerySystemInformation error: %08x, %s"), nt_status, lpMessageBuffer);
...
BOOL SystemHandleInformation::Refresh()
{
	...
	if ( pSysHandleInformation == NULL )
	{
		...
		FormatMessage(
			...
			0, NULL );
		// Display the error message and exit the process
		wxLogError(_T("%s error code: %d, description: %s"), _T("VirtualAlloc"), dw, (LPCTSTR)lpMsgBuf);
		LocalFree(lpMsgBuf);
		return FALSE;
	}
...
{  
	...
	{  
		wxLogError(   _T("OpenProcessToken()   failed,   Error   =   %d   SeDebugPrivilege   is   not   available.\n")   ,   GetLastError()   );  
		return;  
	}  
	if   (   !   LookupPrivilegeValue(   NULL,   SE_DEBUG_NAME,   &sedebugnameValue   )   )  
	{  
		wxLogError(   _T("LookupPrivilegeValue()   failed,   Error   =   %d   SeDebugPrivilege   is   not   available.\n"),   GetLastError()   );  
		CloseHandle(   hToken   );  
		return;  
	}  
	...
	if   (   !   AdjustTokenPrivileges(   hToken,   FALSE,   &tkp,   sizeof   tkp,   NULL,   NULL   )   )  
		wxLogError(   _T("AdjustTokenPrivileges()   failed,   Error   =   %d   SeDebugPrivilege   is   not   available.\n"),   GetLastError()   );  
	CloseHandle(   hToken   );  
}   