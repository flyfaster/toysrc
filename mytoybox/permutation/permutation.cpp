...
void heaps_permutation(vector<int>& nums, int n, vector<vector<int>>& res) {
	...
	for (int i=0; i<n-1; ++i) {
		...
		if(n%2 ==0) swap(nums[n-1], nums[i]);
		else swap(nums[n-1], nums[0]);
	}
	heaps_permutation(nums, n-1, res);
}
...
void check_permutation_algorithm(const char* alg_name, PermutationFunc alg) {
	...
	std::chrono::system_clock::time_point app_start_time = std::chrono::system_clock::now();
	permutation1(nums, nums.size(), res);
	auto duration =
	std::chrono::duration_cast < std::chrono::milliseconds>(std::chrono::system_clock::now() - app_start_time);
	for (auto& vec: res)
		sv.insert(vec);
