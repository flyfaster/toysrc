...
{
	if(cnt++ % DISPLAY_STEP == 1)
  std::cout << __func__ <<" " <<cnt<< " Receive query: " << root << std::endl;
...
{
	if(cnt++ % DISPLAY_STEP == 1)
  std::cout << __func__ <<" " <<cnt<< " Notification: " << root << std::endl;
...
void TestRpc::Init(int argc, char** argv)
{
  cnt=1;
  DISPLAY_STEP=333;
  if(argc>1)
	DISPLAY_STEP=atoi(argv[1]);
}
