...
// [onega@localhost wxWidgets-3.0.2]$ ./configure --enable-stl --with-opengl 2>&1 | tee -a build.txt
// [onega@localhost wxWidgets-3.0.2]$ make 2>&1 | tee -a build.txt
// [onega@localhost wxWidgets-3.0.2]$ sudo make install
// g++ BasicGLPane.cpp -o gl `wx-config --libs --cxxflags --gl-libs` -lGL -lGLU 

// https://wiki.wxwidgets.org/WxGLCanvas#Example
