...
int delay_timer_nsec(uint64_t time_nsec) {
	struct timespec tp;
	uint64_t stamp;
...
int main(int argc, char* argv[]) {
	...
	desc.add_options()("help", "Print help messages")("verbose",
			"enable verbose message")("showlib", "show libraries loaded by me")(
			"profile", "enable profiling functions");

	po::variables_map vm;
	try {
		po::store(po::parse_command_line(argc, argv, desc), vm); // can throw
		if (vm.count("help")) {
			std::cout << argv[0] << " usage:" << std::endl << desc
					<< std::endl;
			return 0;
		}
		po::notify(vm);
	...
	if (vm.count("showlib")) {
		...
		while (!maps.eof()) {
			...
			if (lastspace != std::string::npos) {
				libraries.insert(line.substr(lastspace + 1, -1));
			}
		}
	...
	pcend = rdtsc();
	std::cout << "end-start=" << end - start << " nsec for " << loops
			<< " loops, " << (end - start) / loops
			<< " nsec per call of delay_timer_nsec" << std::endl;
	std::cout << "cpu freq=" << (pcend - pcstart) * 1000 / (end - start)
			<< " MHz\n";
	...
	pcend = rdtsc();
	std::cout << "end-start=" << end - start << " nsec for " << loops
			<< " loops, " << (end - start) / loops << " nsec per call of rdtsc"
			<< std::endl;
	std::cout << "cpu freq=" << (pcend - pcstart) * 1000 / (end - start)
			<< " MHz\n";
	...
	std::cout << "end-start=" << end - start << " nsec for " << loops
			<< " loops, " << (end - start) / loops
			<< " nsec per call of bentime_nsec" << std::endl;
	std::cout << "cpu freq=" << (pcend - pcstart) * 1000 / (end - start)
			<< " MHz\n";
