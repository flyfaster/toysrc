...
int server_main(int argc, char**argv) {
	...
	socklen_t len;
	char mesg[1000];
	std::cout << __func__ << " recvfrom port " << 32000 << std::endl;
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0) {
		std::cerr << __func__ << " failed to set timeout for socket\n";
	}
	sleep(10);
	...
	for (int i = 0; i < 200; i++) {
		...
		if (n < 0) {
			std::cout << __func__ << " timeout, return " << n << " for message " <<i << std::endl;			
			continue;
		}
		mesg[n] = 0;
	...
	std::chrono::milliseconds msduration = std::chrono::duration_cast
			< std::chrono::milliseconds > (duration);
	std::cout << __func__ << " used " << msduration.count() << "ms" << endl;
	return 0;
}
...
int client_main(int argc, char**argv) {
	...
	for (int i = 0; i < 200; i++) {
		int datalen = sprintf(sendline,
				"line%04d, make it long long long long long long long long long long long long long long long long long long long long long to use more space",
				i);
		int actualsent = sendto(sockfd, sendline, strlen(sendline), 0,
				(struct sockaddr *) &servaddr, sizeof(servaddr));
		if(actualsent < datalen)
		std::cout << __func__ << " only sent " << actualsent << " for " << sendline << endl;
	}
...
int main(int argc, char**argv) {
	...
	std::chrono::milliseconds msduration = std::chrono::duration_cast
			< std::chrono::milliseconds > (duration);
	std::cout << argv[0] << " used " << msduration.count() << "ms" << endl;
	return ret;
}
