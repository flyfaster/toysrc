...
int Rijndael::padEncrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer,int lastbuf)
{
...
int Rijndael::padDecrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer,int lastbuf)
{
	int i, numBlocks, padLen;
//	UINT8 block[16];
//	UINT32 iv[4];
			...
			for (i = numBlocks ; i > 0; i--)
			{
				...
				outBuffer += 16;
			      ...
			      if (padLen > 16)return RIJNDAEL_CORRUPTED_DATA;
			        ...
			        {
			         	if(outBuffer[i] != padLen)return RIJNDAEL_CORRUPTED_DATA;
			         }
                        }
...
// main use in unit1
#define FILE_BUF_SIZE (16*2048)

//
//   Encrypt a file (either text or binary file is ok) to a encrypted file,
//   or decrypy a encrypted file to a plainfile.
//   Return the length of the output file.
//

/*
long DealFile(  const  char *InputFile,const  char *OutputFile,Rijndael::Direction dir,UINT8 key[],Rijndael::KeyLength keylen)

{
        ...
        long outFileSize=0;
        FILE *inFile,*outFile;
        unsigned char plainText[FILE_BUF_SIZE];
        unsigned char outputText[FILE_BUF_SIZE+16];
    ...
    if ((rijn_result=rijn.init(Rijndael::ECB,dir,key,keylen))!=RIJNDAEL_SUCCESS)
       return rijn_result;

    if (dir==Rijndael::Encrypt)
     {
      while( (textLen=fread (plainText,1,FILE_BUF_SIZE,inFile))>0)
        {

           outLen=rijn.padEncrypt(plainText,textLen,outputText,feof(inFile));
           if (outLen>0)
            {
             fwrite(outputText,1,outLen,outFile);
             outFileSize+=outLen;
            }
           else
             return (-22);
     ...
     else
      {
        while ((textLen=fread(plainText,1,FILE_BUF_SIZE,inFile))>0)
         {
           outLen=rijn.padDecrypt(plainText,textLen,outputText,feof(inFile));
           if (outLen>0)
             {
              fwrite(outputText,1,outLen,outFile);
              outFileSize+=outLen;
