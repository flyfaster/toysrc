...
//        menu.DestroyMenu();
		...
		CMenu menu;
		POINT curPos;
		const int IDM_RESTORE =1234;
		const int IDM_EXIT =IDM_RESTORE+1;
		menu.CreatePopupMenu(); 
		menu.AppendMenu(MF_STRING|MF_ENABLED,IDM_RESTORE,_T("Restore"));
		menu.AppendMenu(MF_STRING|MF_ENABLED,IDM_EXIT,_T("Exit"));
...
void CSystemTray::RestoreMainWnd()
{
	m_bMinimized_ = false;
	if(m_pMainWnd)
	{
...
void CSystemTray::SetMainWnd(CWnd* pwnd)
{
	m_pMainWnd = pwnd;
}

void CSystemTray::ToTray()
{
	m_bMinimized_ = true;
	if(m_pMainWnd)
	{
