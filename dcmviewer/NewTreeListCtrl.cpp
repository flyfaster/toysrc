...
void CNewTreeListCtrl::OnItemexpanded(NMHDR* pNMHDR, LRESULT* pResult) 
{
	static volatile long event_count = 0;
	LONG count = InterlockedIncrement(&event_count);
	if( count%2 == 0)
	{
		*pResult = 0;
		return;
	...
	if ( pNMTreeView->itemNew.state & TVIS_EXPANDED )
	{
		...
		if(m_msg_handle_wnd)
		{
			::SendMessage(m_msg_handle_wnd,WM_NEWTREE_ITEM_EXPAND,(WPARAM)hitem,0);
			*pResult = 0;
			return;
		}
...
void CNewTreeListCtrl::MemDeleteAllItems(HTREEITEM hParent)
{
	...
	while(hItem!=NULL)
	{
		...
		if(pItem)
			delete pItem;
		CTreeCtrl::SetItemData(hItem, NULL);//http://www.codeproject.com/treectrl/treelist.asp?df=100&forumid=59&exp=0&select=698567
