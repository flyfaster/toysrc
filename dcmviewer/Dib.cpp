...
IStream* CDib::GetStream()
{
	...
	if(pStream)
	{
		...
		bmfh.bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) +
			sizeof(RGBQUAD) * m_nColorTableEntries;	
		ULONG uwritten;
			pStream->Write((LPVOID) &bmfh, sizeof(BITMAPFILEHEADER),&uwritten);
			pStream->Write((LPVOID) m_lpBMIH,  nSizeHdr,&uwritten);
			pStream->Write((LPVOID) m_lpImage, m_dwSizeImage,&uwritten);
			LARGE_INTEGER li;
			li.HighPart = 0;
			li.LowPart = 0;
