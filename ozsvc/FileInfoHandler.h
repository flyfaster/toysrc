...
// request format: command_id(4 bytes), file name.
// response format: length of file content(4 bytes), file content.

class FileInfoHandler: public TcpHandler,
    public boost::enable_shared_from_this<FileInfoHandler>,
    private Loki::PimplOf<FileInfoHandler>::Owner
{
public:
    FileInfoHandler(void);
    ~FileInfoHandler(void);
